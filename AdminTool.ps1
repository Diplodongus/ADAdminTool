######################################################################
# Heavily Modified By: Tanner J. Brouillard
...
if ((Get-ADUser $findusername -Property * | Select-Object -ExpandProperty userSMIMECertificate) -eq $null) {$UserCert = "No"} else {$UserCert = "Yes"}
    ...
    $userlist = Get-ADUser $findusername -Property * | Select-Object *
        ...
        $lbl2.text += "â–ºUser:`t`t`t" + $userlist.DisplayName + "`n"
        $lbl2.text += "â–ºTitle: `t`t`t" + $userlist.Title + "`n"
        $lbl2.text += "â–ºService: `t`t" + $userlist.Company + "`n"
        [int]$pct = (2/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºNationality: `t`t" + $userlist.extensionAttribute4 + "`n"
        $lbl2.text += "â–ºLogin Name: `t`t" + $userlist.UserPrincipalName + "`n"
        $lbl2.text += "â–ºCreated: `t`t" + $userlist.Created + "`n"
        [int]$pct = (3/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºExpiration: `t`t" + $userlist.AccountExpirationDate + "`n"
        $lbl2.text += "â–ºModified: `t`t" + $userlist.Modified + "`n"
        $lbl2.text += "â–ºLast Logon: `t`t" + $userlist.LastLogonDate + "`n"
        $lbl2.text += "â–ºLogon Count: `t`t" + $userlist.LogonCount + "`n"
        [int]$pct = (4/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºEnabled: `t`t" + $userlist.Enabled + "`n"
        $lbl2.text += "â–ºLocked: `t`t" + $userlist.LockedOut + "`n"
        $lbl2.text += "â–ºLockout Time: `t`t" + $userlist.LockOutTime + "`n"
        $lbl2.text += "â–ºEDIPI: `t`t`t" + $userlist.SamAccountName + "`n"
        [int]$pct = (5/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºIA Date: `t`t" + $userlist.iaTrainingDate + "`n"
        $lbl2.text += "â–ºSmartcard Logon: `t" + $userlist.SmartcardLogonRequired + "`n`n"
        $lbl2.text += "â–ºAccount Notes: `t" + $userlist.Description + "`n"
        $lbl2.text += "â–ºManager: `t`t" + ($userlist.Manager -replace '^CN=([^,]+),OU=.+$','$1' | Out-string) + "`n`n"
        $lbl2.text += "â–ºOU Info: `t`t" + $userlist.DistinguishedName + "`n`n"
        $lbl2.text += "â–ºSecurity ID: `t`t" + $userlist.SID + "`n`n"
        $lbl2.text += "â–ºPrimary Group: `t" + $userlist.PrimaryGroup + "`n`n"
        [int]$pct = (6/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºDSN: `t`t`t" + $userlist.OfficePhone + "`n"
        $lbl2.text += "â–ºCountry: `t`t" + $userlist.Country + "`n"
        $lbl2.text += "â–ºBase: `t`t`t" + $userlist.City + "`n"
        $lbl2.text += "â–ºStreet: `t`t" + $userlist.Street + "`n"
        [int]$pct = (7/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºBuilding: `t`t" + $userlist.BuildingName + "`n"
        $lbl2.text += "â–ºRoom: `t`t" + $userlist.RoomNumber + "`n`n"
        $lbl2.text += "â–ºEmail: `t`t`t" + $userlist.mail + "`n"
        $lbl2.text += "â–ºCHES Email: `t`t" + ($userlist.TargetAddress -replace "SMTP:","") + "`n"
        [int]$pct = (8/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºMailbox Created: `t" + $userlist.msExchWhenMailboxCreated + "`n"
        $lbl2.text += "â–ºHas Public Cert: `t" + $UserCert + "`n"
        $lbl2.text += "â–ºHidden From GAL: `t" + $userlist.msExchHideFromAddressLists + "`n`n"
        $lbl2.text += "â–ºSMTP Info (Upper Case 'SMTP' is the Address Being Used): `n" + ($userlist.proxyAddresses | Out-String) + "`n"
        $lbl2.text += "â–ºSecurity Groups " + "(" + ($userlist.MemberOf).count + "):" + "`n" + ($userlist.MemberOf -replace '^CN=([^,]+),OU=.+$','$1' | Out-string) + "`n`n"
        [int]$pct = (9/10)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
        $lbl2.text += "â–ºUser Reported PCs: " + $ReportedPCs
...
$lbl2.text += "â–ºDomain Location:`t " + $PClist.DistinguishedName + "`n`n"
$lbl2.text += "â–ºPhysical Location:`t " + $PClist.Location + "`n`n"
$lbl2.text += "â–ºComputer Name:`t " + $computername + "`n"
$lbl2.text += "â–ºCreation Date:`t`t " + $PClist.WhenCreated + "`n"
        [int]$pct = (10/17)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºLast Changed:`t`t " + $PClist.WhenChanged + "`n"
$lbl2.text += "â–ºLast Logged on:`t " + $PClist.LastLogonDate + "`n"
$lbl2.text += "â–ºEnabled: `t`t " + $PClist.Enabled + "`n"
$lbl2.text += "â–ºLocked: `t`t " + $PClist.LockedOut + "`n"
        [int]$pct = (11/17)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºLockout Time: `t " + $PClist.AccountLockoutTime + "`n"
$lbl2.text += "â–ºMember Of: `t`t " + ($PClist.memberof -replace '^CN=([^,]+),OU=.+$','$1') + "`n"
$lbl2.text += "â–ºManaged By: `t`t " + ($PClist.ManagedBy -replace '^CN=([^,]+),OU=.+$','$1') + "`n"
$lbl2.text += "â–ºNotes: `t`t " + $PClist.Description + "`n`n"
$lbl2.text += "â–ºPassword Last Set:`t " + $PClist.PasswordLastSet + "`n"
$lbl2.text += "â–ºPassword Expires:`t " + $PClist.PasswordNeverExpires + "`n`n"
        [int]$pct = (12/17)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºSID:`t`t`t " + $PClist.SID + "`n"
$lbl2.text += "â–ºAD Object Type:`t " + $PClist.ObjectClass + "`n"
$lbl2.text += "â–ºOperating System:`t " + $PClist.OperatingSystem + "`n"
$lbl2.text += "â–ºOS Version: `t`t " + $PClist.OperatingSystemVersion + "`n"
$lbl2.text += "â–ºLogon Count: `t`t " + $PClist.LogonCount + "`n`n"
        [int]$pct = (13/17)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºIP Address (DNS):`t" + $IPS + "`n"
$lbl2.text += "â–ºOnline: `t`t" + $Pinger + "`n`n`n"
        [int]$pct = (14/17)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
# Local Computer Info #
if ($Pinger -eq $true){
$lbl2.text += "`t`t`t`t`t`t     ~ Local Computer Info ~ `n`n"
$lbl2.text += "â–ºCurrent User:`t`t " + (([adsisearcher]"(&(objectCategory=user)(sAMAccountName=$rComp1))").FindAll().Properties.cn) + "`n"
$lbl2.text += "â–ºLast User:`t`t " + (([adsisearcher]"(&(objectCategory=user)(sAMAccountName=$rComp3))").FindAll().Properties.cn) + "`n"
$lbl2.text += "â–ºUser logged on for:`t " + $ULD + "`n"
$lbl2.text += "â–ºLast Restart:`t`t " + (Compare-DateTime -TimeOfObject $rOS -Property "Lastbootuptime") + "`n"
$lbl2.text += "â–ºPending Restart:`t " + $RestartStatus + "`n"
$lbl2.text += "â–ºRSAT/ADUC:`t`t " + $RSAT + "`n`n"
$lbl2.text += "â–ºManufacturer:`t`t " + $rComp.Manufacturer + "`n"
$lbl2.text += "â–ºModel:`t`t " + $rComp.Model + "`n"
        [int]$pct = (15/17)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºSKU Number:`t`t " + $rComp.SystemSKUNumber + "`n"
$lbl2.text += "â–ºSerial:`t`t`t " + $rBIOS.SerialNumber + "`n`n"
$lbl2.text += "â–ºCPU:`t`t`t " + $rCPU.Name.Trim() + "`n"
$lbl2.text += "â–ºRAM:`t`t`t " + $mem + "`n"
$lbl2.text += "â–ºHard Drive: `t`t {0:N1} GB Free / {1:N1} GB Total `n" -f $HDfree, $HDsize
$lbl2.text += "â–ºOptical Drive:`t`t " + "(" + $rDVD.Drive + ") " + $rDVD.Caption + "`n"
$lbl2.text += "â–ºVideo Card:`t`t " + $rVid.Name + "`n"
$lbl2.text += "â–ºMonitor(s):`t`t " + $monitor + "`n`n"
$lbl2.text += "â–ºLocal Date/Time:`t " + $localdate + "`n"
$lbl2.text += "â–ºOperating System:`t " + $rOS.Caption + "`n"
$lbl2.text += "â–ºSDC Version:`t`t " + $SDC + "`n"
        [int]$pct = (16/17)*100        #set percentage
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºOS Version:`t`t " + $rOS.Version + "`n"
$lbl2.text += "â–ºService Pack:`t`t " + $rOS.CSDVersion + "`n"
$lbl2.text += "â–ºOS Architecture:`t " + $rComp.SystemType + "`n"
$lbl2.text += "â–ºPC imaged on:`t`t " + $imagedate + "`n`n"
$lbl2.text += "â–ºIP Address:`t`t" + $IP + "`n"
$lbl2.text += "â–ºSubnet:`t`t" + $Subnet + "`n"
$lbl2.text += "â–ºGateway:`t`t" + $GW + "`n"
$lbl2.text += "â–ºDNS:`t`t`t" + $DNS + "`n`n"
$lbl2.text += "â–ºNetwork Adapters and MAC Addresses:`t" + ($rMAC | Out-String) + "`n"
}
else{
$lbl2.text += "`t`t`t`t`t`t     ~ Local Computer Info ~ `n`n"
...
$lbl2.text += "â–ºLocked: `t`t " + $PClist.LockedOut + "`n"
        ...
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºLockout Time: `t " + $PClist.AccountLockoutTime + "`n"
$lbl2.text += "â–ºMemberOf: `t`t " + ($PClist.memberof -replace '^CN=([^,]+),OU=.+$','$1') + "`n"
$lbl2.text += "â–ºManaged By: `t`t " + ($PClist.ManagedBy -replace '^CN=([^,]+),OU=.+$','$1') + "`n"
$lbl2.text += "â–ºNotes: `t`t " + $PClist.Description + "`n`n"
$lbl2.text += "â–ºPassword Last Set:`t " + $PClist.PasswordLastSet + "`n"
$lbl2.text += "â–ºPassword Expires:`t " + $PClist.PasswordNeverExpires + "`n`n"
...
$lbl2.text += "â–ºLogon Count: `t`t " + $PClist.LogonCount + "`n`n"
        ...
        $progress1.Value = $pct        #update the progress bar
$lbl2.text += "â–ºIP Address (DNS):`t" + $IPS + "`n"
$lbl2.text += "â–ºOnline: `t`t" + $Pinger
...
$lbl2.text +=  "`n"
$lbl2.text +=  "`n"
$lbl2.text += "â–ºComputer Name:`t " + $rComp.name + "`n"
$lbl2.text += "â–ºOperating System:`t " + $rOS.Caption + "`n"
$lbl2.text += "â–ºSDC Version:`t`t " + $SDC + "`n`n"
$lbl2.text += "â–ºLast Restart:`t`t " + (Compare-DateTime -TimeOfObject $rOS -Property "Lastbootuptime") + "`n`n"
$lbl2.text += "â–ºSCCM Installed:`t " + $IsInstalledCM + "`n"
...
$lbl2.text += "â–ºTanium Installed:`t " + $IsInstalledTa + "`n"
...
$lbl2.text += "â–ºMcAfee Installed:`t " + $IsInstalledMc + "`n"
...
$lbl2.text += "â–ºEPO Server:`t`t " + $EPOSer + "`n`n"
$lbl2.text += "â–ºUpdate Server:`t`t " + $UpdateSer + "`n`n"
$lbl2.text += "â–ºLast 10 Windows Updates:`t " + $rUpdate + "`n`n"
...
$Userss = Get-ADGroupMember -Identity â€œ$SearcherInputâ€ | Select-Object Name
